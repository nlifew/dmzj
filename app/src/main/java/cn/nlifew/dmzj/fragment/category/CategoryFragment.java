...
import static cn.nlifew.dmzj.fragment.category.CategoryViewModel.ComicWrapper.TYPE_LOAD_MORE;
import static cn.nlifew.dmzj.fragment.category.CategoryViewModel.ComicWrapper.TYPE_REFRESH;
...
public class CategoryFragment extends BaseLoadMoreFragment implements
    ...
    private void onCategoryChanged(CategoryBean[] bean) {
        if (bean == null) {
            return;
        }
    ...
    private void onComicListChanged(CategoryViewModel.ComicWrapper wrapper) {
        if (wrapper == null) {
            return;
        }
        switch (wrapper.type) {
    ...
    @Override
    public void onCategorySelect(CategoryBean bean, int index) {
        setLoading(true);
